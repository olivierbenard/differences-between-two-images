...
@click.command()
@click.option('-di', '--display-images', is_flag=True, help='Display the images given as an input')
@click.option('-dssim', '--display-ssim', is_flag=True, help='Display the computed ssim between reference and images')
@click.option('-bd', '--bound-differences', is_flag=True, help='Highlight the differences by bounding them into a rectangle')
@click.option('-dd', '--draw-differences', is_flag=True, help='Highlight the differences by drawing them')
...
def main(display_images, display_ssim, bound_differences, draw_differences, images):
    ...
    for image in images:
        ...
        if not initialized: # initialisation with the first image
            ...
            (score, diff) = ssim(ref_gray_frame, gray, full=True) # compute ssim between two images
            print(f'Similarity between {ref_name} and {filename}:', score)

            if display_ssim:
                cv2.imshow(f'difference-between-{ref_name}-and-{filename}', diff)

            diff = (diff * 255).astype("uint8") # to detect contours, must be converted to  8-bit unsigned integers array in the [0,255] range
            ...
            if bound_differences:
                cv2.imshow(f'bound-differences-between-{ref_name}-and-{filename}-on-{ref_name}', temp_ref_frame) # display the image on screen
                cv2.imshow(f'bound-differences-between-{ref_name}-and-{filename}-on-{filename}', frame) # display the image on screen
            if draw_differences:
                cv2.imshow(f'draw-differences-between-{ref_name}-and-{filename}', filled_after) # display the image on screen
